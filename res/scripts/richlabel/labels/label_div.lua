...
local function div_createbmfont(self, word, fnt, fontcolor)
	if word == "" or fnt == "" then 
		return
	end
	local label = cc.Label:createWithBMFont(fnt, word)
	if not label then 
		self:printf("<div> - create bmfont label failed " .. fnt)
		return
	end
	label:setColor(fontcolor)
	return label
...
return function (self, params, default)
	...
	local fnt = params.fnt or ""

	if fnt ~= "" then
		for index, char in pairs(charlist) do
			local label = div_createbmfont(self, char, fnt, fontcolor)
			if label then
