...
function copyCsbNode(csbNode)
    local newNode = cc.Node.create()
    for _, node in ipairs(csbNode:getChildren()) do
        if node.clone then
...
function TestUIPageViewScene:createPageView()
    self.pv = cc.ui.UIPageView.new {
        ...
        :addTo(self)
        v = cc.rect(80, 80, 780, 480)
        if v.clone then
            print("can be clone")
        end
        if iskindof(self.pv, "Node") then
            print('iskindof UIWidget')
        else
            print('no iskindof UIWidget')
        end
         print(tolua.iskindof(self.pv, "Widget"))
     print(type(self.pv))
    DumpNode(self.pv)
    -- add items
    for i=1,18 do
