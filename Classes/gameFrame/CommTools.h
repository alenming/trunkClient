...
template<typename T>
inline void releaseAndClearSet(std::set<T>& l)
{
    for (typename std::set<T>::iterator iter = l.begin();
        iter != l.end(); ++iter)
    {
        SAFE_RELEASE((*iter));
    }
...
template<typename T>
inline void deleteAndClearSet(std::set<T>& v)
{
    for (typename std::set<T>::iterator iter = v.begin();
        iter != v.end(); ++iter)
    {
        SAFE_DELETE((*iter));
    }
...
template<typename T>
inline void releaseAndClearDeque(std::deque<T>& l)
{
    for (typename std::deque<T>::iterator iter = l.begin();
        iter != l.end(); ++iter)
    {
        SAFE_RELEASE((*iter));
    }
...
template<typename T>
inline void deleteAndClearDeque(std::deque<T>& v)
{
    for (typename std::deque<T>::iterator iter = v.begin();
        iter != v.end(); ++iter)
    {
        SAFE_DELETE((*iter));
    }
...
template<class T>
inline T toolToNum(const std::string& str)
{
...
inline std::string gb23122utf8(const char* gb2312)
{
