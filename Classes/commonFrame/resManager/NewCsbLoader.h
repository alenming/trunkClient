...
*
*/
#ifndef __NEW_CSB_LOADER_H__
#define __NEW_CSB_LOADER_H__
...
class NewCsbLoader : public IResLoader
{
public:
	NewCsbLoader();
	virtual ~NewCsbLoader();
	...
	bool addPreloadRes(const std::string& csbFile, const ResLoadedCallback& callBack = nullptr);
	// 异步加载预加载队列中的资源
...
private:
	// 创建一个Csb节点
	cocos2d::Node* createCsbNode(const NewCsbLoadingInfo& csbInfo);
...
private:
	...
	std::map<std::string, NewCsbLoadingInfo> m_LoadingQueue;         // 异步加载队列
